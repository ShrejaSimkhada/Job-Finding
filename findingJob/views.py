...
def editUserProfile(request):
    ...
    context = {'user': user}
    return render(request, 'findingJob/edit_user_profile.html', context)
...
def jobList(request):
    jobListinpart = jobdetail.objects.filter(type='Part Time')
    jobListinfull = jobdetail.objects.filter(type='Full Time')
    context = {'jobListinfull': jobListinfull, 'jobListinaprt': jobListinpart}
    return render(request, 'findingJob/job-list.html', context)


def jobDetail(request, jobid):

    fav_job = get_object_or_404(jobdetail, id=jobid)
    fav= bool
    if fav_job.favourites.filter(id=request.user.id).exists():
        fav = True

    jobinfo = jobdetail.objects.filter(id=jobid)
    context = {'jobinfo': jobinfo[0], 'fav':fav}
    return render(request, 'findingJob/job-detail.html', context)


@login_required(login_url='login')
def favourite_add(request, id):
    fav_job = get_object_or_404(jobdetail, id=id)
    if fav_job.favourites.filter(id=request.user.id).exists():
        fav_job.favourites.remove(request.user)
    else:
        fav_job.favourites.add(request.user)
    return HttpResponseRedirect(request.META['HTTP_REFERER'])


@login_required(login_url='login')
def favourite_list(request):
    fav_list = jobdetail.objects.filter(favourites=request.user)
    return render(request, 'findingJob/favourites-list.html', {'fav_list': fav_list})
